...
def settings_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    for key in ("APP_ENV", "OPEN_ROUTER_API_KEY", "DATABASE_URL", "LOG_LEVEL"):
...
def test_defaults_to_dev_when_app_env_unset(settings_dir: Path) -> None:
    ...
    assert config.sql_max_limit == 50
    assert config.search_limit == 7
    assert config.sql_result_truncation == 200
