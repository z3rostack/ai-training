...
class Config(BaseSettings):
    ...
    sqlite_path: str = Field(default="agent.db", validation_alias="SQLITE_PATH")
    ...
    sql_result_truncation: int = Field(
        default=8000,
        validation_alias="SQL_RESULT_TRUNCATION",
    )
    llm_max_retries: int = Field(
        default=2,
        validation_alias="LLM_MAX_RETRIES",
    )
    human_review_max_retries: int = Field(
        default=3,
        validation_alias="HUMAN_REVIEW_MAX_RETRIES",
    )
    log_level: str = "DEBUG"
